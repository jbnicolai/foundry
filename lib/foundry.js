...
extend(Foundry.prototype, {
  ...
  bindCommands: function () {
    ...
    program
      ...
      .option('-y', 'Automatically confirm release prompt')
      .action(function callRelease (version, argv) {
        ...
        Foundry.getReleaseLibs({
          ...
          // Define mechanism to release via
          function runRelease() {
          ...
          // If we need to verify the plugins/release, prompt them
          if (!argv.y) {
            // Prompt the user to verify they want to release
            var releaseNames = releaseLibs.map(function outputReleaseLib (releaseLib) {
              return releaseLib._name + '@' + releaseLib._version;
            }).join(', ');
            console.log(Command.prototype);
            Command.prototype.confirm('Releasing with ' + releaseNames + '. Continue? [Yn]', function runRelease (confirmed) {
              console.log(confirmed);
            });
          // Otherwise, run the release
          } else {
            runRelease();
          }
        });
